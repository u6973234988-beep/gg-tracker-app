...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, ...props }, ref) => (
    <input
      type={type}
      className={cn(
        'flex h-10 w-full rounded-md border border-[#1e1e2e] bg-[#12121a] px-3 py-2 text-sm text-white placeholder:text-gray-500 focus:border-[#7F00FF] focus:outline-none focus:ring-2 focus:ring-[#7F00FF] focus:ring-opacity-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-[#1e1e2e] dark:bg-[#12121a] dark:text-white dark:placeholder:text-gray-500 dark:focus:border-[#7F00FF] dark:focus:ring-[#7F00FF]',
        className,
      )}
      ref={ref}
      {...props}
