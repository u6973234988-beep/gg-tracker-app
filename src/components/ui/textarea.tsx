...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, ...props }, ref) => (
    <textarea
      className={cn(
        'flex min-h-[80px] w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-[#7F00FF] focus:outline-none focus:ring-2 focus:ring-[#7F00FF] focus:ring-opacity-50 disabled:cursor-not-allowed disabled:opacity-50 resize-none dark:border-[#1e1e2e] dark:bg-[#12121a] dark:text-white dark:placeholder:text-gray-500 dark:focus:border-[#7F00FF] dark:focus:ring-[#7F00FF]',
        className,
      )}
      ref={ref}
      {...props}
